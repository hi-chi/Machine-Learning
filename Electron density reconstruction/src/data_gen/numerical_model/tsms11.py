...
from src.data_gen.utils.save_dataset import save_year11
from src.data_gen.utils.particle_tsms11 import Particle
from src.data_gen.utils import geometry_tsms11
...
from src.preprocessing.image import tsms11_cut_out_i1, tsms11_cut_out_i2
...
def beam(Erange, sig_x, sig_y):
    ...
    ang = [
        ...
        10 + 50.0 * random.random(),
        0.7e-3 * (1 + 2 * random.random())
    ]
...
def gen_beams():
    a_x = 0.20 * (-0.35 + 1.35 * random.random())       # [-0.07, 0.2]
    a_y = 0.03 * (-1.00 + 2.00 * random.random())       # [-0.03, 0.03]
...
@njit
def simulation(pgen):
    screen1 = np.zeros((geometry_tsms11.ny1, geometry_tsms11.nx1))
    screen2 = np.zeros((geometry_tsms11.ny2, geometry_tsms11.nx2))
    ...
    for scat_angle, energy, alpha, beta in pgen.iterate():
        data.append(energy)
        v0, absV0 = part.velFromMevAngle(energy, alpha, beta)
        ...
        t1 = (sec(0, 0, v0[2] / v0[1], geometry_tsms11.rmag1, 0, geometry_tsms11.l1)) / v0[2]
        r0 += v0 * t1

        # in 1 magnet
        while np.sqrt((r0[2] - geometry_tsms11.l1) ** 2 + r0[1] ** 2) < (geometry_tsms11.rmag1 + 0.001):
            res = part.push(part.magnet1(r0, [0., 0., geometry_tsms11.l1]), r0, v0, dt)
            r0 = np.ascontiguousarray(res[:3])
            v0 = np.ascontiguousarray(res[3:])

        # to the first screen
        t2 = (sec_scr(r0[1], r0[2], v0[2] / v0[1], 1, 0, geometry_tsms11.l3) - (r0[2])) / v0[2]
        r0 = r0 + v0 * t2

        yi = int((r0[1] - geometry_tsms11.x_min1) / geometry_tsms11.dx1)
        xi = int((r0[0] - geometry_tsms11.y_min1) / geometry_tsms11.dx1)

        seed = 4
        if seed <= xi < geometry_tsms11.ny1 - seed and seed <= yi < geometry_tsms11.nx1 - seed:
        ...
        t3 = (sec(r0[1], r0[2], v0[2] / v0[1], geometry_tsms11.rmag2, 0, geometry_tsms11.l2) - r0[2]) / v0[2]
        r0 += v0 * t3

        # in 2 magnets
        while np.sqrt((r0[2] - geometry_tsms11.l2) ** 2 + r0[1] ** 2) < (geometry_tsms11.rmag2 + 0.001):
            res = part.push(part.magnet2(r0, [0., 0., geometry_tsms11.l2]), r0, v0, dt)
            r0 = np.ascontiguousarray(res[:3])
            v0 = np.ascontiguousarray(res[3:])

        # to the second screen
        t4 = (sec_scr(r0[1], r0[2], v0[2] / v0[1], 1, 0, geometry_tsms11.l4) - (r0[2])) / v0[2]
        r0 += v0 * t4

        yi = int((r0[1] - geometry_tsms11.x_min2) / geometry_tsms11.dx2)
        xi = int((r0[0] - geometry_tsms11.y_min2) / geometry_tsms11.dx2)

        seed = 4
        if seed <= xi < geometry_tsms11.ny2 - seed and seed <= yi < geometry_tsms11.nx2 - seed:
            for i_n in range(-seed, seed):
                for j_n in range(-seed, seed):
...
def generate_pg__(mode, N):
    ...
    if mode == 0:
        ...
        samples, x, y, z = get_samples(distribution, N)
        pgen = Generator3DBeam(samples, x, y, z, data_config['ANGLE_COEFFICIENT'],  data_config['BASELINE_ANGLE'])
    else:
...
def generate(idx, save_args, mode, N):
    ...
    save_year11(path, save_args, i1, i2, angle_x, angle_y, spectrum, angle_dist)
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        screen1 = tsms11_cut_out_i1(screen1)
        screen2 = tsms11_cut_out_i2(screen2)
