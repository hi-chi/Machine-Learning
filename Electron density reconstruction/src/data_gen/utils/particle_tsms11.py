...
class Particle:
    ...
    def velFromMevAngle(self, E=100, alf=0, bet=0):
    ...
    def magnet2(self, r=[0,0,0], rc=[0,0,0]):
            arg=(0.715*np.exp(-(np.sqrt((r[1]-rc[1])**2+(r[2]-rc[2])**2))**7/0.031**7)-0.025-0.055*np.exp(-(np.sqrt((r[1]-rc[1])**2+(r[2]-rc[2])**2)-0.045)**2/0.02**2))
            return arg
    
    def magnet1(self, r=[0,0,0], rc=[0,0,0]):
            arg=(0.45*np.exp(-(np.sqrt((r[1]-rc[1])**2+(r[2]-rc[2])**2))**7/0.061**7)-0.025-0.055*np.exp(-(np.sqrt((r[1]-rc[1])**2+(r[2]-rc[2])**2)-0.045)**2/0.02**2))
            return arg
